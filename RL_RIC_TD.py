...
r, mu, sigma = 0.10, 0.03, 0.20
h_coef, p_coef = 1.0, 1.0
Kp, kp, Km, km = 2.0, 0.5, 2.0, 0.5
...
def train_randomised_td(lam1, lam2, initial_psi="psi0",
    ...
    for n in range(N_outer):
        ...
        with torch.no_grad():
            ...
            psi_eval = psi_eval.cpu()
            psi_classical_net = psi_classical_net.cpu()
            relative_l2_loss = torch.norm(psi_eval(xx) - psi_classical_net(xx)) / torch.norm(psi_classical_net(xx))
        ...
        if n % 1 == 0 or n == N_outer-1:
            with torch.no_grad():
                ...
                plt.close("all")
            torch.save({"model_state": psi_net.state_dict(),"optimizer_state": opt.state_dict()}, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_psi", type=str, default="psi0", help="initial_psi")
